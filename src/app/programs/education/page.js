...
export const metadata = {
  title: 'Education Programs | WDO',
  description: 'WDO initiatives focused on improving education in Somaliland.',
}
    ...
    <div className="animate-fade-in">
      <section className="section-light" style={{ padding: '60px 0', borderBottom: '1px solid #eeeeee', backgroundColor: '#f8fafc' }}>
        <div className="container">
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', color: '#0056b3', marginBottom: '15px' }}>
            <GraduationCap size={30} />
            <span style={{ fontWeight: '800', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '1px' }}>Strategic Pillar</span>
          </div>
          <h1 style={{ fontSize: '2.5rem', fontWeight: '900', color: '#1e293b', margin: 0 }}>IMPROVING EDUCATION</h1>
          <p style={{ color: '#64748b', marginTop: '10px', fontSize: '1.1rem', maxWidth: '800px' }}>
            Enhancing educational access for vulnerable households and climate-affected communities through innovative technologies, community resources, and professional training.
          </p>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          ...
          {programs && programs.length > 0 ? (
              ...
              {programs.map((program) => (
                <div key={program.id} className="card program-card" style={{ border: '1px solid #eee', borderRadius: '12px', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '20px' }} className="card-padding">
                    ...
                    <Link href={`/programs/${program.id}`} style={{ color: '#0056b3', fontWeight: '800', fontSize: '0.85rem', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '5px' }} className="card-link">
            ...
            <div style={{ textAlign: 'center', padding: '60px', backgroundColor: '#f8fafc', borderRadius: '24px' }}>
              <p style={{ color: '#64748b' }}>We are currently documenting our ongoing education projects. Please check back soon.</p>
            </div>
          )}
        </div>
      </section>
